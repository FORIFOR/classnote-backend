...
- **AI 機能 (Gemini)**:
    ...
    - QA (質問応答)
- **文字起こし**: 
    - **Streaming STT V2**: Binary Protocol による低遅延・高精度なリアルタイム文字起こし
    - **Batch STT**: 長時間音声のバックグラウンド処理
- **セキュリティ & コストガード (v0.4.0〜)**:
    - **Cloud Ticket System**: クラウド処理用の時限式チケットによる厳格な認可
    - **120分ハードリミット**: 全クラウド処理に対する最大利用時間の強制 (コスト爆発防止)
    - **リスクスコアリング**: 不審な挙動の自動検知とアカウント制限 (Restricted/Blocked)
    - **Freeプラン クレジット制**: 初回1回限定のクラウドお試し枠の管理
