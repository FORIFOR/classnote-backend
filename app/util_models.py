...
class UpdateSessionRequest(BaseModel):
    ...
    # Duration and timing (set by iOS when recording ends)
...
class ImportYouTubeRequest(BaseModel):
    ...
    captionUrl: Optional[str] = None  # iOS-provided timedtext baseUrl (pre-signed)
...
class JobResponse(BaseModel):
    ...
    # [FIX 2026-02] Default to now() to prevent 500 on cache hit path
...
class WatchTranscriptPartialRequest(BaseModel):
    """Watch録音フローの軽量パーシャルトランスクリプト更新"""
    transcriptDraft: str
    seq: int
    durationSec: Optional[float] = None
    source: str = "device_sherpa"

class WatchTranscriptPartialResponse(BaseModel):
    sessionId: str
    accepted: bool
    serverSeq: int
    transcriptDraftLen: int
...
class MeResponse(BaseModel):
    ...
    # [Security] Tracking (time-window based risk scoring)
    securityState: str = "normal"       # normal / watch / restricted / blocked
    riskScore: int = 0                  # effectiveRisk (computed from counters, not accumulated)
    lastEvaluatedAt: Optional[datetime] = None
    lastRestrictedAt: Optional[datetime] = None
    lastBlockedAt: Optional[datetime] = None
    ...
    # [NEW 2026-02] Subscription Status (for UI display)
    # - subscriptionAutoRenews: False = user cancelled, but still valid until expiresAt
    # - subscriptionExpiresAt: when the current billing period ends
    # - subscriptionStatus: "active" | "cancelled" | "grace_period" | "billing_retry" | "expired"
    subscriptionAutoRenews: Optional[bool] = None
    subscriptionExpiresAt: Optional[datetime] = None
    subscriptionStatus: Optional[str] = None

    # [NEW 2026-02] Invite system
    inviteCode: Optional[str] = None        # User's own invite code
...
class BootstrapRequest(BaseModel):
    """Request for POST /bootstrap — single startup endpoint."""
    appAccountToken: Optional[str] = None  # Apple StoreKit2 app account token


class BootstrapResponse(BaseModel):
    """Response for POST /bootstrap — replaces /me/lite + conditional /auth/canonicalize."""
    # Identity
    uid: str
    accountId: str
    plan: str = "free"
    ...
    # Feature gates (plan-based, no usage check)
    featureGates: FeatureGates = Field(default_factory=FeatureGates)
    ...
    # Canonicalize result (if merge happened)
    canonicalized: bool = False
    previousAccountId: Optional[str] = None

    # Custom claims
    claimsRefreshRequired: bool = False  # true → client should call getIDToken(forceRefresh: true)

    # Async repair flags (server schedules repairs in background)
    repairNeeded: bool = False
    repairTasks: List[str] = []  # e.g. ["sync_phone", "fix_uid_link", "add_provider"]
...
class UserProfileResponse(BaseModel):
    ...
    shareCode: Optional[str] = None
    isShareable: bool = True
    customSummaryPrompt: Optional[str] = None
    customQuizPrompt: Optional[str] = None
...
class SessionDetailResponse(SessionResponse):
    ...
    quizStatus: Optional[JobStatus] = None
