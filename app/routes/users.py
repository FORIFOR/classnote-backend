...
@router.post("/me/apple_app_account_token")
async def set_apple_token(req: AppleTokenReq, current_user: CurrentUser = Depends(get_current_user)):
    ...
    This token is used to bind the initial subscription claim to this user account.
    """
    ...
    # Use set(merge=True) to ensure creation if missing (though unexpected for auth'd user)
    user_ref.set({
        "appleAppAccountToken": req.appAccountToken, 
...
async def get_me(
    ...
    # True only if:
    # - Token DOES NOT have phone_number
    # - AND DB DOES NOT have phoneE164 (legacy fallback)
    # - AND user is not a "phone" provider (which implies phone_number presence usually, but we check token first)
    has_phone_in_token = bool(token_phone)
    has_phone_in_db = bool(phone_in_db)
    
    needs_phone = False
    if not has_phone_in_token and not has_phone_in_db:
         needs_phone = True
    ...
    return MeResponse(
        ...
        freeCloudCreditsRemaining=cloud_remaining,  # [FIX] Use calculated value
        freeSummaryCreditsRemaining=summary_remaining,  # [FIX] Use calculated value
        freeQuizCreditsRemaining=quiz_remaining,  # [FIX] Use calculated value
        cloud=None,
