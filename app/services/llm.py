...
def _get_summary_schema(mode: str) -> dict:
    # translate mode reuses the meeting schema (decisions / todos / highlights
    # structure fits bilingual session summaries better than the lecture shape).
    if mode == "meeting" or mode == "translate":
        return _SCHEMA_MEETING_SUMMARY
    return _SCHEMA_LECTURE_SUMMARY
...
"""
    if mode == "translate":
        # Translate sessions may contain ===ORIGINAL===/===TRANSLATION=== delimiters.
        # The quick summary is always produced in Japanese regardless of input language.
...
def _build_summary_tags_prompt(
    ...
    *,
    ...
    if mode == "lecture":
        ...
        base = _build_translate_summary_prompt(text)
    else:
...
- timeline: 5〜8件。会話全体を時系列にカバーする章立て。
  - `startSec` < `endSec`。不明な秒数は推定でよいが、会話順は必ず守る。
  - `title` は体言止めで短く具体的（最大20文字）。「議論」「会話」だけの抽象語は禁止。
  - `summary` は1〜2文（最大120文字）。その章で話されたことを要約する。
  - `topicChange: true` はその章で話題が明確に切り替わった時のみ。それ以外は false。
  - 話題転換が少ないなら無理に分割しない（最小3件）。隣接章と内容が重複しないように。
- tags: 2〜5個
...
{text}
"""
def _build_translate_summary_prompt(
    text: str,
) -> str:
    """翻訳セッション専用の要約プロンプト。

    Input transcript may contain ``===ORIGINAL===\\n...\\n===TRANSLATION===\\n...``
    delimiters (original-language speech + Japanese translation). The prompt
    explicitly asks the LLM to use BOTH sides to produce a richer Japanese
    summary rather than discarding the translation.

    Structure mirrors the meeting prompt (decisions/todos/highlights) because
    translated meetings / interviews / talks share that structure. What differs
    is the translate-specific guidance:
      - produce output in Japanese regardless of original language
      - when a key term appears in the original, preserve it with a Japanese
        gloss (e.g. "due diligence（買収前調査）")
      - don't mix original-language phrases into the structured output
    """
    return f"""あなたは逐次通訳・翻訳セッションの編集者です。以下の文字起こしは、原語パート
と日本語訳パートが `===ORIGINAL===` / `===TRANSLATION===` で区切られている場合があります。
両方を参照して、**日本語で** セッション全体の要点を構造化JSONにまとめてください。

# 絶対ルール（厳守）
1) 原文（原語・訳文どちらでも）に無い固有名詞・数値・日付・期限は一切書かない。
2) 期限は原文に具体日付がある場合のみ due="YYYY-MM-DD"。それ以外は "期限不明"。本文には
   「今月中」「来週」などの原文の曖昧表現を残して良い。
3) ASRノイズや誤訳と判断した語は捨てる。確信の無い固有名詞は書かない。
4) 出力は必ず日本語。原語表記は「原語（日本語訳）」形式で key term のみ残して良い
   （例: "due diligence（買収前調査）"）。
5) 「結論は明確でない」は decisions=0 かつ todos=0 の時だけ許可。それ以外は禁止。
6) JSON内テキストに Markdown記法（**太字**, ***強調***, ##見出し, - 箇条書き 等）は
   一切使わない。構造はJSONで表現済みなので、text/overview等の値は平文で書く。

# バイリンガル素材の使い方
- ORIGINAL と TRANSLATION の両方を根拠として読む。一方だけに出てくる情報も漏らさない。
- 訳文に抜けがある（要約や省略が強い）場合は原語側の情報を日本語で補って要約して良い。
- 話者が明確なら participants に残す。通訳者は participants に入れない。
- 重要な key term（業界用語・固有名詞）は keyTerms に「原語（日本語訳）」で最大10件まで残す。
- highlights の1件目は必ず [decision] の形、2件目は必ず [todo] の形（原文から抽出した
  "結論" / "次の一手"）。結論・次の一手が本当に無い場合は [info] の要点文で埋めて良い。

# "薄さ防止" の必須要件
- highlights: 5〜10件。原文の重要ポイントを全て拾う。
- decisions: 原文に「合意/決定/予定」があれば最大6件。無ければ 0 件で良いが、
  その場合 outcomeStatus="discussion_only" を必ず使う。
- todos: 原文から拾える次アクションを 3〜6件（1件だけにしない）。
- 変更点（やめる/見直す/方針転換）は highlights に [info] または [change] として残す。
- 優先順位の合意（〜より先に〜する）は todos の priority に反映する。

# 作業手順（内部、最終出力には書かない）
Step1) 原語パートと訳文パートから共通の事実（decisions / todos / changes /
       priorities / discussionTopics / keyTerms）を抽出。
       片側だけにある情報も拾う。
Step2) Step1 の材料だけで下記 JSON を日本語で生成。
...
{{
  "summary": {{
    "type": "translate",
    "highlights": [
      {{"text": "今回の結論: （日本語で具体的に1行）", "category": "decision", "needConfirm": false}},
      {{"text": "次の一手: （日本語で具体的に1行）", "category": "todo", "needConfirm": false}}
    ],
    "bottomLine": "5〜10文・400〜800文字。翻訳セッション全体を日本語で網羅する結論文。原語側のみに存在する情報も日本語で補い、主要トピック・判断背景・変更点・合意事項を全て含める。",
    "overview": "800〜1500文字。セッションの背景・主要テーマ・参加者発言の流れを日本語で順に要約する。原語と訳文の両方を参照した結果として書く。読点で区切り、1文を短くする。Markdown禁止。",
    "outcomeStatus": "decided | partially_decided | discussion_only | blocked",
    "whyItMatters": "1文。このセッションが何の判断に効くか（日本語）",
    "decisions": [
      {{"text": "〜することに決定（日本語）", "owner": "担当or不明", "due": "YYYY-MM-DD|期限不明", "status": "confirmed|tentative|inferred", "ownerSource": "explicit|inferred|unknown", "dueSource": "explicit|inferred|unknown", "reason": "決定の背景を1文", "confidence": 0.9, "evidenceHint": "原文の短い引用", "needConfirm": false}}
    ],
    "todos": [
      {{"text": "成果物ベースのTODO（日本語、具体的に）", "owner": "担当or不明", "due": "YYYY-MM-DD|期限不明", "priority": "high|mid|low", "blocking": "false", "ownerSource": "explicit|inferred|unknown", "dueSource": "explicit|inferred|unknown", "confidence": 0.85, "evidenceHint": "原文の短い引用", "needConfirm": false}}
    ],
    "openQuestions": [
      {{"text": "未決事項（日本語）", "impact": "high|mid|low", "needConfirm": true}}
    ],
    "keyTerms": [
      {{"original": "due diligence", "japanese": "買収前調査", "note": "原文で繰り返し出現", "needConfirm": false}}
    ],
    "keywords": [{{"text": "日本語のタグ候補"}}],
    "participants": [
      {{"name": "話者名 or 不明（通訳者は除く）", "role": "Speaker|Other|不明"}}
    ],
    "discussionPoints": [
      {{"topic": "トピック名", "summary": "要約（日本語）", "conclusion": "「合意」or「保留（要確認）」", "nextAction": "対応するTODO", "needConfirm": false}}
    ],
    "timeline": [
      {{"startSec": 0, "endSec": 180, "title": "導入（体言止め20文字以内）", "summary": "章の要約・1〜2文・120文字以内", "topicChange": true}}
    ],
    "uiHints": {{
      "topFocus": ["decisions", "todos", "keyTerms"],
      "tone": "business",
      "suggestedBadges": ["要確認", "期限不明", "担当不明"]
    }}
  }},
  "tags": ["日本語タグ1", "日本語タグ2"],
  "suggestedTitle": "10〜25文字の体言止め日本語タイトル"
}}

# 制約
- highlights 5〜10件。先頭2件は [decision] と [todo]。[discussion] は使わない。
- decisions 最大6件。合意・予定・承認がある限り全て拾う。無ければ 0 件。
- todos 3〜6件（原文にあれば必ず出す。1件だけにしない）。
- openQuestions 0〜5件。
- keyTerms 0〜10件。業界用語/固有名詞/頻出語で原語と日本語を併記する価値のあるものだけ。
- discussionPoints 3〜8件。議論された全トピックをカバー。
- timeline 4〜8件。`title` は体言止め最大20文字、`summary` は最大120文字。
- tags 2〜5個（日本語）。
- suggestedTitle: 10〜25文字、体言止め、助詞で終わらない（例:「Q2戦略ミーティング議事録」）。
...
- **結論ファースト**: bottomLine を最初に読むだけで「この講義で結局何を理解すべきか / なぜ重要か」が分かるように圧縮する。抽象的な一般論ではなく、具体の核心を述べる。

# UI目的
- 最上部に「結論（bottomLine）」を 2〜5 文で先出しし、続けて whyItMatters を 1 文で補う
- 次に「今日のポイントのおさらい」を3〜7行で簡潔に（highlights）
- その次に「用語カード」「流れ（セクション）」「重要式・手順」「例題/範囲」を見やすく
...
- bottomLine は overview と役割が違う（bottomLine=核心の圧縮、overview=網羅的詳細）。両方を別々に生成する。

# 出力JSONスキーマ（厳守）
{{
  "summary": {{
    "type": "lecture",
    "bottomLine": "2〜5文（200〜400文字）。この講義の核心を結論ファーストで圧縮する。『この講義で結局何を理解すべきか』『何が試験・応用・次回講義に効くか』を具体的に述べる。抽象語（「重要な内容を学んだ」等）禁止。固有名詞・定義・数値を含める。",
    "whyItMatters": "1文（40〜80文字）。その理解がなぜ重要か（試験/応用/次の単元への接続/実務）。",
    "outcomeStatus": "comprehensive | partial | outline_only",
    "highlights": [
...
# 制約
- bottomLine: 必須。2〜5文（200〜400文字）。この講義の核心を先に述べる。抽象的結論（「重要な内容を学んだ」等）は禁止。具体の結論・適用先・条件を含める。講義が途中で終わっている等で網羅できない場合は `outcomeStatus="partial"` または `"outline_only"`、文言にも「本講義ではXまでを扱い、Yは次回」と明示。
- whyItMatters: 必須。1文（40〜80文字）。試験/応用/次の単元/実務のどれに効くかを短く示す。
- outcomeStatus: 必須。`comprehensive`（網羅できた）/ `partial`（一部のみカバー）/ `outline_only`（概要止まり）のいずれか。
...
- timeline: 5〜8件。講義を時系列に沿って章立てする。`sections` と別物で、sections は学習単位、timeline は audio seek 用の時間軸。
  - `startSec` < `endSec`。秒数不明なら推定でよいが、順序は守る。
  - `title` は体言止め・最大20文字（例:「線形性の導入」「部分積分の例題」）。「講義」「解説」だけの抽象語は禁止。
  - `summary` は1〜2文（最大120文字）。その時間帯で扱った内容を要約する。
  - `topicChange: true` は明確な単元切り替え時のみ。
  - 話題転換が少ないなら最小3件。隣接章と内容が重複しないように。
...
_MAP_PROMPT_TRANSLATE = """あなたは逐次通訳セッションの整理アシスタントです。以下の文字起こし（一部分）は
原語と日本語訳が `===ORIGINAL===` / `===TRANSLATION===` で区切られている場合があります。
両方を読んで、"原文にある事実だけ" を日本語で抽出してください。

# 絶対ルール
1) 原語・訳文どちらにも無い情報は一切書かない。
2) 出力は日本語。原語の key term を残したい場合のみ「原語（日本語訳）」形式で text 内に残す。
3) evidenceHint は原語 or 訳文の短い引用フレーズを必ず付ける（捏造防止の根拠）。
4) ASRノイズや誤訳は捨てる。確信が無ければ書かない。
5) 通訳者自身の発言（「はい、それでは〜と言っています」等）は事実として抽出しない。
...
# type ガイド
- decision: 実行が確定した決定のみ
- todo: 次アクション（成果物ベースで具体的に）
- open-question: 未決事項
- key-point: 方針 / 暫定合意 / 優先順位
- risk: リスクや依存関係
- change: 方針の見直し
- priority: 優先順位の合意
- key-term: 業界用語 / 固有名詞で原語と日本語を併記する価値のあるもの
- info: 背景共有・前提条件

# 出力ルール（厳守）
- JSON 形式のみ出力。前置き / 説明 / Markdown 禁止。
- facts: 最大25件。text は 1〜2文・最大150文字・日本語。
- evidenceHint は原語または訳文からの短い引用（最大30文字）。
- key-term の場合は text に "原語（日本語訳）" 形式で書く。
...
_REDUCE_PROMPT_TRANSLATE = """あなたは逐次通訳セッションの編集者です。以下は原語と日本語訳の両方から
抽出された evidence 付き facts です。
視聴者が最初に知りたいのは「結局、何が話され、何が決まったか」である。日本語で構造化JSONに整理せよ。

# 入力（抽出済み facts — evidenceHint が根拠）
{extracted_points}

# 絶対ルール（厳守）
1) 入力 facts に無い固有名詞・数値・日付・期限は一切書かない。
2) 出力は日本語。key-term type の fact は `keyTerms` に「原語（日本語訳）」形式で残せる。
3) due は具体日付がある時のみ。それ以外は "期限不明"。曖昧な期限表現は text に残して良い。
4) 「結論は明確でない」は decisions=0 かつ todos=0 の時だけ許可。outcomeStatus="discussion_only" を使う。
5) ASRノイズや誤訳は捨てる。
6) highlights には [decision][todo][info][risk][open_question][change] を使う。[discussion] 禁止。
7) JSON 内テキストに Markdown 記法（**太字**, ***強調***, ##見出し 等）は一切使わない。平文で書く。
8) ownerSource/dueSource: 原文（原語または訳文）で明示されている場合 "explicit"、推定なら "inferred"、不明なら "unknown"。

# 設計思想
- bottomLine を読むだけで「何が話され、何が決まり、次に何をするか」が分かるように書く。
- 結論が出ていないセッションでは無理に結論化しない。重要な未決は openQuestions に送る。
- 原語と訳文の両方を参照して得た情報は、**日本語で** 統合して書く。

# "薄さ防止" の必須要件
- highlights 先頭2行は: A) [decision] 今回の結論、B) [todo] 次の一手。
- todos 3〜6件（1件だけにしない）。
- type="purchase" / "change" / "priority" の facts は decisions / todos / priority 欄に反映する。
- keyTerms は key-term 型の facts を最大10件まで残す。

# 整合チェック→整形
Step1（内部作業）: facts の evidenceHint を見て根拠が確かなものだけ採用。重複は統合。
Step2: Step1 の材料だけで下記 JSON 形式で日本語出力。

# 出力スキーマ
{{
  "summary": {{
    "type": "translate",
    "bottomLine": "5〜10文・400〜800文字の日本語。セッション全体を網羅する。何が話され/決まり/次に何をするかに加え、原語側のみに存在する情報も日本語で補う。",
    "outcomeStatus": "decided | partially_decided | discussion_only | blocked",
    "whyItMatters": "1文。このセッションが何の判断に効くか（日本語）。",
    "highlights": [
      {{"text": "今回の結論: （日本語で具体的に1行）", "category": "decision", "needConfirm": false}},
    ...
    "decisions": [{{
      ...
      "reason": "なぜこの決定に至ったか（1文）",
      "confidence": 0.9,
      "evidenceHint": "根拠となる原語 or 訳文の引用フレーズ",
      "needConfirm": false
    }}],
    "todos": [{{
      "text": "成果物ベースのTODO（日本語で具体的に）",
      "owner": "担当者名 or 担当不明",
      "ownerSource": "explicit | inferred | unknown",
      "due": "YYYY-MM-DD | 期限不明",
      "dueSource": "explicit | inferred | unknown",
    ...
    "openQuestions": [{{"text": "未決事項（日本語）", "impact": "high|mid|low", "needConfirm": true}}],
    "keyTerms": [{{"original": "due diligence", "japanese": "買収前調査", "note": "繰り返し出現", "needConfirm": false}}],
    "keywords": [{{"text": "日本語タグ候補"}}],
    "participants": [{{"name": "話者名 or 不明（通訳者は除く）", "role": "Speaker|Other|不明"}}],
    "discussionPoints": [{{"topic": "トピック名", "summary": "要約（日本語）", "conclusion": "合意|保留（要確認）", "nextAction": "TODOと一致", "needConfirm": false}}],
    "timeline": [{{"startSec": 0, "endSec": 180, "title": "導入（体言止め20文字以内）", "summary": "1〜2文・120文字以内", "topicChange": true}}],
    "uiHints": {{"topFocus": ["decisions", "todos", "keyTerms"], "tone": "business", "suggestedBadges": ["要確認", "期限不明", "担当不明"]}}
  }},
  "tags": ["日本語タグ1", "日本語タグ2"],
  "suggestedTitle": "10〜25文字の体言止め日本語タイトル"
}}
...
# 制約
- highlights 5〜10件、先頭 [decision]+[todo]、[discussion] 禁止。
- decisions 最大8件。合意・予定・承認は全て拾う。
- todos 3〜8件（1件だけにしない）。
- openQuestions 0〜5件。
- keyTerms 0〜10件。業界用語・固有名詞のみ。
- timeline 4〜8件。title は20字以内の体言止め、summary は120字以内。
- tags 2〜5個（日本語）。
- suggestedTitle: 10〜25文字、体言止め、助詞で終わらない。
"""


def _pick_map_prompt(mode: str) -> str:
    if mode == "lecture":
        return _MAP_PROMPT_LECTURE
    if mode == "translate":
        return _MAP_PROMPT_TRANSLATE
    return _MAP_PROMPT_MEETING


def _pick_reduce_prompt(mode: str) -> str:
    if mode == "lecture":
        return _REDUCE_PROMPT_LECTURE
    if mode == "translate":
        return _REDUCE_PROMPT_TRANSLATE
    return _REDUCE_PROMPT_MEETING
...
async def _map_extract_chunk(chunk: str, mode: str, chunk_index: int) -> dict:
    ...
    prompt_template = _pick_map_prompt(mode)
...
async def _reduce_synthesize(
    ...
    prompt_template = _pick_reduce_prompt(mode)
    ...
    # meeting / translate share the stricter business-mode temperature
    reduce_temp = 0.3 if mode in ("meeting", "translate") else 0.5
    resp = await _timed_llm_call(
        _model,
        prompt,
        GenerationConfig(
...
def _normalize_summary_json(data: dict, mode: str) -> dict:
    ...
    if mode == "lecture":
        # Conclusion-first fields (lecture vocabulary mirrors meeting structure
        # so PPTX/PDF hero callouts can share the same `bottom_line` /
        # `why_it_matters` / `outcome_status` rendering path).
        ...
        if os_val not in ("comprehensive", "partial", "outline_only"):
            # Back-compat: tolerate legacy meeting-style values by mapping.
            if os_val in ("decided", "partially_decided"):
                normalized["outcomeStatus"] = "comprehensive"
            elif os_val == "discussion_only":
                normalized["outcomeStatus"] = "partial"
            elif os_val == "blocked":
                normalized["outcomeStatus"] = "outline_only"
            else:
                normalized["outcomeStatus"] = "comprehensive"
        else:
    ...
    # timeline (v2): validate + repair LLM output. Drops items missing title/summary,
    # coerces startSec/endSec to int, enforces startSec < endSec, sorts chronologically,
    # dedupes on (startSec, title), caps at 8 items. Empty list triggers the
    # rule-based fallback in app/services/summary_postprocess.fallback_timeline.
    _timeline_raw = _coerce_list(normalized.get("timeline"))
    _timeline_clean: List[dict] = []
    for _item in _timeline_raw:
        ...
        if not isinstance(_summary, str) or not _summary.strip():
            continue
        try:
            _start = max(0, int(float(_item.get("startSec") or 0)))
        except (TypeError, ValueError):
            _start = 0
        try:
            _end = int(float(_item.get("endSec") or 0))
        except (TypeError, ValueError):
            _end = _start
        if _end <= _start:
            _end = _start + 1
        _timeline_clean.append({
            "startSec": _start,
            "endSec": _end,
            "startMs": _start * 1000,
            "endMs": _end * 1000,
            "title": _title.strip()[:30],
            "summary": _summary.strip()[:200],
            "topicChange": bool(_item.get("topicChange")),
            "fallback": False,
        })
    _timeline_clean.sort(key=lambda x: (x["startSec"], x["endSec"]))
    _seen: set[tuple[int, str]] = set()
    _deduped: List[dict] = []
    for _item in _timeline_clean:
        _key = (_item["startSec"], _item["title"])
        if _key in _seen:
            continue
        _seen.add(_key)
        _deduped.append(_item)
    normalized["timeline"] = _deduped[:8]
...
def _validate_summary_json(data: dict, mode: str, source_len: int) -> tuple[bool, str]:
    ...
    # lecture mode: prefer bottomLine (conclusion-first), fall back to overview
    # for backward compatibility with pre-2026-04-19 lecture summaries.
    bottom_line = data.get("bottomLine")
    if isinstance(bottom_line, str) and len(bottom_line) >= 10:
        return True, "ok"
