...
#### Hard Rule — share_drafts 再利用方針 (重複実装の防止)

Phase 2-A の変換系コマンド (メール文 / Slack 投稿文 / アジェンダ / リマインド文) は、
**LINE/Slack chat 専用ロジックに閉じ込めない**。後続の `share_drafts` MVP
(`POST /v1/sessions/{id}/share-drafts:generate`) からも再利用できる pure service
として実装する:

- 新サービス `app/services/share_draft_service.py` を作成し、`generate_share_draft(...)`
  pure function を置く(LLM 呼び出しは引数で制御、デフォルトはテンプレートベース)
- `assistant_hub` / LINE / Slack の chat handler は、自前で文面整形をせず、
  この service を呼ぶだけにする
- Desktop / REST / iOS から将来同じ生成ロジックが叩けるよう、引数は
  `(session, summary_v2, next_actions, channel, audience, format, include)`
  の組合せで受ける
- 出力は dict (Pydantic 化は share_drafts MVP 側で実施) で `{title, body,
  source: {summaryV2Version, transcriptVersion}, channel, format}` を返す

これを守らないと、後の share_drafts MVP で LINE 用 / Slack 用 / メール用 / Desktop 用
の似た文章生成ロジックが 4 箇所に重複する。
